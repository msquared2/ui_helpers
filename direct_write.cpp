...
void TextFormat::set_emoji_processing_config(std::optional<EmojiProcessingConfig> emoji_processing_config)
{
    if (!emoji_processing_config) {
    ...
    try {
        ...
        m_font_fallback = create_emoji_font_fallback(font_collection, std::move(system_font_fallback),
            emoji_processing_config->colour_emoji_family_name.c_str(),
            emoji_processing_config->monochrome_emoji_family_name.c_str());
    }
    CATCH_LOG()
}
